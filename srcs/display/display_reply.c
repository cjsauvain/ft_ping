...
static void display_non_verbose_output(t_reply_pckt reply_pckt, t_ping_stats stats)
{
...
void	display_reply(t_ping *ping)
{
	if (ping->reply_pckt.icmp_pckt.icmphdr.type == 0)
		display_non_verbose_output(ping->reply_pckt, ping->stats);

	else if (display_icmp_message(ping->reply_pckt, ping->verbose_mode))
